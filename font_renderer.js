...
pc.script.create('font_renderer', function (context) {
    ...
    Font_renderer.prototype = {
        initialize: function () {
            ...
            var onAssetsReady = function () {
                if (!this.atlas || !this.font) return;
            ...
            var fontAtlas = context.assets.get(this.fontAtlas);
            fontAtlas.ready(function (asset) {
                this.atlas = asset.resource;
                onAssetsReady();
            }.bind(this));

            var fontJson = context.assets.get(this.fontJson);
            fontJson.ready(function (asset) {
                this.font = asset.resource;
                onAssetsReady();
            }.bind(this));

            context.assets.load(fontAtlas);
            context.assets.load(fontJson);
        ...
        onClick: function (cursor) {
            ...
            bry = 2.0 * (scaling.y * (-this.font.common.base) + offset.y) / resolution.y - 1.0;
