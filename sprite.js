    ...
    Sprite.prototype = {
        initialize: function () {
           var canvas = document.getElementById('application-canvas');
            ...
            this.command = new pc.scene.Command(pc.scene.LAYER_HUD, pc.scene.BLEND_NORMAL, function () {
                if (this.entity.enabled && this.texture) {
                    // Set the shader
                    gd.setShader(shader);
            ...
            this.command.key = this.depth;
            context.scene.drawCalls.push(this.command);

            // Load assets
            if (this.textureAsset) {
                this.setTextureAsset(context.assets.get(this.textureAsset));
            }
        ...
        setTextureAsset: function (asset) {
            if (! asset) return;

            this.textureAsset = asset.id;
