...
pc.script.create('game', function (app) {
    ...
    Game.prototype = {
        ...
        initialize: function () {
            ...
            // set to true if we're building for FB microgame
            this.microgame = true;

            this.player = app.root.findByName('Bird').script.player;
            this.environment = app.root.findByName('Environment');
            this.bonusEffect = app.root.findByName('BonusEffect');
            this.physics = this.entity.script.physics;
            this.input = this.entity.script.input;
            this.patterns = this.entity.script.patterns;
            this.particles = app.root.findByName('Particles').script.particles;
            this.storage = this.entity.script.storage;
            this.audio = app.root.findByName('Audio').script.audio;
            this.scaleTweener = this.entity.script.batch_tween_scale;
            this.splashEntities = app.root.findByName('SplashScreen');
            ...
            // raise highscore event on startup to submit on fb etc.
            var highscore = this.getHighScore();
            if (highscore) {
                app.on('fb:login', function () {
                    app.fire('highscore', highscore);
                });
            }
        },
        ...
        getHighScore: function () {
            return this.storage.loadNumber('score', 0);
        },
        ...
        gameover: function () {
            if (!this.isOver) {
                ...
                if (this.score > 0) {
                    var previousScore = this.getHighScore();
